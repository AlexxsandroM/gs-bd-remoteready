...
-- Remover tabelas
BEGIN EXECUTE IMMEDIATE 'DROP TABLE TB_GS_POST_LEITURA CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN NULL; END;
/
...
/
BEGIN EXECUTE IMMEDIATE 'DROP SEQUENCE SEQ_TB_GS_LEITURA'; EXCEPTION WHEN OTHERS THEN NULL; END;
/
...
CREATE OR REPLACE PACKAGE PKG_REMOTEREADY AS
    ...
    PROCEDURE PRC_REGISTRAR_LEITURA(
        P_ID_USUARIO IN NUMBER,
        P_ID_POST    IN NUMBER,
        P_TEMPO_SEG  IN NUMBER DEFAULT 60
    );
    ...
    -- ========================================================================
    -- FUNÇÃO 2: VALIDAÇÃO COM REGEXP (Requisito: 15 pontos)
    -- Calcula compatibilidade do usuário com mercado remoto
    -- ========================================================================
...
CREATE OR REPLACE PACKAGE BODY PKG_REMOTEREADY AS
    ...
    BEGIN
        ...
        IF P_TIPO_HISTORICO IN ('COMPLETO', 'CHAT') THEN
            ...
            FOR chat_rec IN (
                SELECT ID_CHAT, SUBSTR(PROMPT, 1, 50) AS PROMPT_SHORT, DT_CRIACAO
    ...
    BEGIN
        ...
        INSERT INTO TB_GS_CHAT_HISTORY (
            ID_USUARIO, PROMPT, RESPONSE, DT_CRIACAO
        ) VALUES (
    ...
    BEGIN
        ...
        FOR chat_rec IN (
            SELECT ID_CHAT, SUBSTR(PROMPT, 1, 100) AS PROMPT_SHORT, 
                   SUBSTR(NVL(RESPONSE, 'Sem resposta'), 1, 100) AS RESPONSE_SHORT,
    ...
    BEGIN
        ...
        BEGIN
            INSERT INTO TB_GS_POST_LEITURA (
                ID_LEITURA, ID_USUARIO, ID_POST, QT_TEMPO_SEG
            ) VALUES (
                SEQ_TB_GS_LEITURA.NEXTVAL, P_ID_USUARIO, P_ID_POST, P_TEMPO_SEG
            ...
            DBMS_OUTPUT.PUT_LINE('Leitura registrada: ID=' || V_ID_LEITURA);
        ...
        -- Contar total de leituras do usuário
    ...
    -- ========================================================================
    -- FUNÇÃO 2.2: CALCULAR COMPATIBILIDADE (Requisito: 15 pontos)
    -- Calcula % de compatibilidade do usuário com mercado remoto
    -- Baseado em: posts lidos, certificados, tempo de plataforma
    -- ========================================================================
    FUNCTION FN_CALC_COMPATIBILIDADE(
        P_ID_USUARIO IN NUMBER
    ) RETURN NUMBER AS
        V_POSTS_LIDOS NUMBER;
        V_CERTIFICADOS NUMBER;
        V_DIAS_PLATAFORMA NUMBER;
        V_PONTOS NUMBER := 0;
        V_COMPATIBILIDADE NUMBER;
    BEGIN
        -- Contar posts lidos
        ...
        -- Dias desde cadastro
        SELECT TRUNC(SYSDATE - DT_CRIACAO) INTO V_DIAS_PLATAFORMA
        FROM TB_GS_USUARIO
        WHERE ID_USUARIO = P_ID_USUARIO;
        
        -- Cálculo de pontos (máximo 100)
        V_PONTOS := V_PONTOS + LEAST(V_POSTS_LIDOS * 5, 50);      -- Máx 50 pts (10 posts)
        V_PONTOS := V_PONTOS + LEAST(V_CERTIFICADOS * 10, 30);    -- Máx 30 pts (3 certs)
        V_PONTOS := V_PONTOS + LEAST(V_DIAS_PLATAFORMA / 3, 20);  -- Máx 20 pts (60 dias)
        
        V_COMPATIBILIDADE := LEAST(V_PONTOS, 100);
        
        RETURN V_COMPATIBILIDADE;
...
BEGIN
    ...
    PKG_REMOTEREADY.PRC_REGISTRAR_LEITURA(V_ID_USER1, V_ID_POST1, 180);
    PKG_REMOTEREADY.PRC_REGISTRAR_LEITURA(V_ID_USER1, V_ID_POST2, 240);
    PKG_REMOTEREADY.PRC_REGISTRAR_LEITURA(V_ID_USER1, V_ID_POST3, 120);
    
    -- Registrar leituras (Maria lê 1 post)
    PKG_REMOTEREADY.PRC_REGISTRAR_LEITURA(V_ID_USER2, V_ID_POST1, 150);
...
PROMPT
PROMPT === TESTE 3: Cálculo de Compatibilidade ===;
SELECT 
    U.NM_USUARIO,
    PKG_REMOTEREADY.FN_CALC_COMPATIBILIDADE(U.ID_USUARIO) AS COMPATIBILIDADE_PCT
